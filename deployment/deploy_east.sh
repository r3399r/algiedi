...
npm ci
npm run pre:deploy
aws cloudformation package --template-file aws/cloudformation/ap-east-1.yaml --output-template-file packaged.yaml --s3-bucket gotron-cf-midway-ap-east-1
aws cloudformation deploy --template-file packaged.yaml --stack-name $project-$env-stack --parameter-overrides TargetEnvr=$env Project=$project SubDomain=$subDomain Domain=$domain --no-fail-on-empty-changeset --s3-bucket gotron-cf-midway-ap-east-1
echo ====================================================================================
